describe('mock XMLHttpRequest', () => {
    ...
    it('mocks responses after send', function() {
        ...
        expect(jasmine.Ajax.requests.filter(/users/)[0].url)
          .toBe('/users/get/33');

        // Get request by filter function
        expect(jasmine.Ajax.requests
                 .filter((request) => { return request.method === 'GET'; })[0]
                 .method)
          .toBe('GET');

        // responseTimeout (need to install mock clock - will move it 30s),
        // responseError
        jasmine.Ajax.requests.mostRecent().respondWith({
            ...
            responseType : 'json',
            responseText :
              JSON.stringify({ name : 'mark', id : 'mark@zzo.com' }),
        });

        expect(this.callback).toHaveBeenCalledWith(jasmine.objectContaining({
            name : 'mark',
        }));
    });
    ...
    describe('stub responses', function() {
        it('success response', function() {
            ...
            var response = {
                results : [ { color : 'green', size : 'large', id : 1234 } ],
            };
            jasmine.Ajax.stubRequest('/products/find', request, 'POST')
              .andReturn({
                  status : 200,
                  contentType : 'application/json',
            ...
            expect(this.callback)
              .toHaveBeenCalledWith(jasmine.objectContaining(response));

            expect(this.callback.calls.count()).toEqual(1);
            expect(jasmine.Ajax.requests.mostRecent().data())
              .toEqual(JSON.parse(request));
    ...
    it('one shot', function() {
        jasmine.Ajax.withMock(function() {
            ...
            expect(callback).toHaveBeenCalledWith(
              jasmine.objectContaining(response));
